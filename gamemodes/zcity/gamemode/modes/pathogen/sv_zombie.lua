MODE.name = "pathogen"
MODE.PrintName = "Pathogen (Zombies)"
MODE.start_time = 0
MODE.end_time = 10
...
util.AddNetworkString("zb_zombriefing")
util.AddNetworkString("zb_zomzombriefing")
util.AddNetworkString("zb_zomtraitorbriefing")
...
util.AddNetworkString("ZB_Pathogen_RoundEnd")

function MODE:CreateTimer(name, delay, repetitions, func)
	self.saved.Timers = self.saved.Timers or {}

	if timer.Exists(name) then
		print("Pathogen: Attempted to create an already existing timer: " .. name)
		return
	end
...
function MODE:FigureOutConsequences()
	...
	for _, v in ipairs(self.saved.Players or {}) do
		...
		if self.saved.Zombified[ply] or role == "zombie" then
			data[ply].now.role = "zombie"
		end

		//it's a hack but whatever :3
		if data[ply].now.role == "zombie" and data[ply].now.escaped then
	...
	if table.Count(self.saved.Escaped) == 0 and #self.humans == 0 then
		WinCondition = 1 // zombies win
	elseif #self.zombies == 0 or table.Count(self.saved.Escaped) > 0 then
	...
	net.Start("ZB_Pathogen_RoundEnd")
...
function MODE:SetupTraitor(ply)
	...
	net.Start("zb_zomtraitorbriefing")
	net.Send(ply)
...
function MODE:RoundStart()
	...
	self.saved.zomamount = math.max(math.Round(#players / 4), 1)
	for i, ply in RandomPairs(players) do
		if self.saved.zoms[ply] or ply:Team() == TEAM_SPECTATOR then continue end

		self.saved.zoms[ply] = true

		if table.Count(self.saved.zoms) >= self.saved.zomamount then
			break
		end
	end
	...
	for i, ply in RandomPairs(players) do
		...
		if self.saved.traitors[ply] or self.saved.zoms[ply] or ply:Team() == TEAM_SPECTATOR then continue end
	...
	for _, ply in player.Iterator() do
		if ply:Team() == TEAM_SPECTATOR then continue end
		if self.saved.zoms[ply] then continue end
	...
	for ply, _ in pairs(self.saved.zoms) do  // set them up later from everyone else so that they spawn the furthest from everyone
...
function MODE:BroadcastContractor(text)
	...
	for _, v in ipairs(self.saved.traitorsLookUp) do
		...
		if v.PlayerClassName == "zombie" then continue end
...
function MODE:ShouldRoundEnd()
	...
	for _, v in player.Iterator() do
		...
		if v.PlayerClassName != "zombie" and v.PlayerClassName != "commanderforces" then
			table.insert(MODE.humans, v)
		elseif v.PlayerClassName != "commanderforces" then
			table.insert(MODE.zombies, v)
		end
	end
	...
	return #MODE.humans == 0 or #MODE.zombies == 0
end
	...
	timer.Simple(.1,function()
		...
		if last_attacker then
			if math.random(1, 3) == 1 then
				if last_attacker.PlayerClassName != "zombie" then
					last_attacker:Notify(table.Random(killzombies))
				end
			end
		end
	end)
		...
		for i, weld in pairs(constraint.FindConstraints(this, "Weld")) do
			...
			if weld.Ent2:IsRagdoll() then
				...
				if ply and !table.HasValue(players, ply) then
					if ply.PlayerClassName != "zombie" and ply.PlayerClassName != "commanderforces" then
						extracted = true
					end
...
function MODE:ZB_LootMultiplier(ply)
	local zomamount = self.saved.zomamount

	if ply.PlayerClassName == "zombie" then
		return #MODE.humans * 2
	else
		return #MODE.zombies * 0.5
	end
end

function MODE:HG_OZombified(ply)
	self.saved.Zombified[ply] = true
end
...
function MODE:Think()
	...
	if time > self.LastExtractThink then
		for _, ply in player.Iterator() do
			if !ply:Alive() then continue end
			if ply.PlayerClassName == "zombie" or ply.PlayerClassName == "commanderforces" then continue end
			...
			if self.saved.TraitorExtractPoint and self.saved.traitors and self.saved.traitors[ply] then
				if (ply:GetPos() - self.saved.TraitorExtractPoint):LengthSqr() < 40000 then
					if !timer.Exists("zb_Pathogen_long_plyextract_" .. ply:EntIndex()) then
						ply:SetLocalVar("zb_Pathogen_Extraction", time + 10)
						timer.Create("zb_Pathogen_long_plyextract_" .. ply:EntIndex(), 10, 1, function()
							if (ply:GetPos() - self.saved.TraitorExtractPoint):LengthSqr() < 40000 then
				...
				elseif ply:GetLocalVar("zb_Pathogen_Extraction") then
					ply:SetLocalVar("zb_Pathogen_Extraction", nil)

					if timer.Exists("zb_Pathogen_long_plyextract_" .. ply:EntIndex()) then
						timer.Remove("zb_Pathogen_long_plyextract_" .. ply:EntIndex())
					end
				end
