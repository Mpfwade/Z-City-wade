...
local RandomEvents = {
    ["Sneeze"] = function( owner, org )
        owner:EmitSound(ThatPlyIsFemale(owner) and "zcitysnd/female/sneez_"..math.random(1,4)..".mp3" or "zcitysnd/male/sneez_"..math.random(1,4)..".mp3", nil, 100 + ((owner.PlayerClassName == "furry" or owner.PlayerClassName == "zombie") and 20 or 0))
    ...
    ["Hungry"] = function( owner, org )
        owner:EmitSound("zcitysnd/uni/hungry_"..math.random(1,6)..".mp3", nil, 100 + ((owner.PlayerClassName == "furry" or owner.PlayerClassName == "zombie") and 20 or 0))
    end,
    ["Burp"] = function( owner, org )
        owner:EmitSound("snd_jack_hmcd_burp.wav", nil, 100 + ((owner.PlayerClassName == "furry" or owner.PlayerClassName == "zombie") and 20 or 0))
        for i = 1, 10 do
    ...
    ["Cough"] = function( owner, org )
        owner:EmitSound(ThatPlyIsFemale(owner) and "zcitysnd/female/cough_"..math.random(1,6)..".mp3" or "zcitysnd/male/cough_"..math.random(1,6)..".mp3",75,100 + ((owner.PlayerClassName == "furry" or owner.PlayerClassName == "zombie") and 20 or 0),1)
        timer.Simple(.3,function()
...
module[2] = function(owner, org, timeValue)
    --print("huy")
    if org.timeToRandom < CurTime() and owner:IsPlayer() and owner:Alive() and owner.GetPlayerClass and owner:GetPlayerClass() and owner:GetPlayerClass().CanEmitRNDSound then
